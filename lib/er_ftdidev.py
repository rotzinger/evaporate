...
# Only port A is used in the moment, eeprom changed to pid 8011
...
class er_output(object):
    def __init__(self):
        VRange = 10 # 5, 10
        self.DAC = AD5422(0x403, 0x8011, 1)
        self.DAC.reset_DAC()
        self,DAC.config_DAC(on=True,VRange=VRange)
    def set_output(Vvalue):
        value = int(Vvalue/float(self.VRange)*65536) # 16 bit resolution
        self.DAC.IO(value)
...
if __name__ == "__main__":
    ...
    DAC.reset_DAC()
    DAC.config_DAC(on=True,VRange=10)
